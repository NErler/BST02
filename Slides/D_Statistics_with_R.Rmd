...
\item Modeling non-linear effects
\end{itemize}
...
\item \blue{Kolmogoriv-Smirnov Test:} \texttt{ks.test()}\newline
...
\begin{column}{0.5\linewidth}
\blue{Note:}\\
When the link is not explicitely specified (i.e. options 2-4),
...
ggplot(plotDF, aes(x = resid)) +
  geom_histogram(bins = 100, fill = EMCdark, alpha = 0.5, color = EMCdark) +
  xlab('Residuals') +
  temp,

ggplot(plotDF, aes(x = fitted, y = resid)) +
  geom_point(color = EMCdark, alpha = 0.5) +
...
ggplot(data.frame(r = quantile(rstandard(lm_mod), seq(0, 1, length = 200)),
  ...
  geom_point(color = EMCdark, alpha = 0.5, size = 2) +
  temp +
...
\begin{block}{Demo}
\begin{itemize}
\item Model Evaluation \button{https://nerler.github.io/BST02/demo/statistics-with-r/Model_Evaluation.R}{R} \button{https://nerler.github.io/BST02/demo/statistics-with-r/Model_Evaluation.html}{R}
\end{itemize}
\end{block}
\end{column}
\end{columns}
...
Non-linear effects may be \blue{more complex} than can be modeled with a simple
