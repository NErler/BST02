...
* In order to list the created variables use the following functions
```{r, eval = FALSE, include = TRUE, echo = TRUE}
objects()
```
...
* Specialized functions for importing data from other statistical programs
...
* Specialized functions for export data to other statistical programs
...
\item \texttt{write.csv()}, \texttt{write.xlsx()}, \texttt{write.table()}
\end{itemize}
...
* For position indexing, use dublicated positive values to select the same elements
```{r, eval = TRUE, include = TRUE, echo = TRUE}
x <- c(6:17)
x
x[c(2,2,2)]
