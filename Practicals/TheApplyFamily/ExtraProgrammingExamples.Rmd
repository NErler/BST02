...
Define a function (with the name `mysummary`) that takes one parameter (let’s call it `x`) that investigates if it is a data frame a factor or a numeric variable and returm the message "This is a data.frame" if `x` is a data frame, "This is a factor" is `x` is a factor and "This is numeric" if `x` is numeric.  
...
Use an if statement. When you want to display results from the function you cant to use the print(...) function.
</div>')
```
...
The previous function `mysummary` still does not do anything usefull when we apply it on the whole data.frame. Extend the function to take again one parameter (let’s call it `x`) but to return also some descriptive statistics for the data frame. 
...
Create a for loop that goes through the columns of the **retinopathy** data set and plots each column. If the column is a numerical variable then use a density plot, if the column is categorical variable then use a barchart. Create a plot with multiple panel
...
For each data set (**heart** and **retinopathy**), select only the patients that had an event.
...
Use a for loop. Note that the variable name of the events is diffent for the two data sets.
</div>')
```
