...
### Extra Task:
How does the model interpretation change if we use the ordinal version of `stage`?
```{r}
pbc$stage_ord <- factor(pbc$stage, ordered = TRUE) # or as.ordered()

mod1c <- glm(spiders ~ age + sex + chol + bili + stage_ord + ascites, data = pbc,
            family = binomial())
summary(mod1c)
