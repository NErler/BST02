...
### Dataset 
For this practical, we will use the **pbc** dataset from the package **survival**.
...
<br><br>
**Interpretation fo the effect of `bili`:**
```{r, echo = FALSE}
...
When comparing models with regards to the effect of a variable that is modeled
...
### Task 4
The non-linear association for `bili` resemples the shape of the logarithmic function.
...
matplot(contrasts(pbc$stage_ord), type = 'l')
matplot(contrasts(pbc$stage_ord))
lines(x, -1.118034 + 0.4472136 * x)
lines(x, 0.5 * (x - 2.5)^2 - 0.625, xlim = c(1, 4), ylim = c(-1, 1), col = 2)
```


### Investigate the problem with stage
```{r}
table(pbc$stage, pbc$spiders, exclude = NULL)

# but we also might have missing values in other variables, so better make 
# sure we also exclude these cases from our table
...
mod1b <- glm(spiders ~ age + sex + chol + bili + stage_rev + ascites, data = pbc,
             family = binomial())
summary(mod1b)
