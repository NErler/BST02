...
Define a function (with the name `mysummary`) that takes one parameter (let’s call it `x`) that investigates if it is a data frame a factor or a numeric variable and return the message "This is a data.frame" if `x` is a data frame, "This is a factor" is `x` is a factor and "This is numeric" if `x` is numeric.  
