...
* Obtain the mean of the columns `age`, `futime`, `risk` and **retinopathy** data sets. 
...
Create the following function `DerivativeFunction <- function(x) {log10(x) + 10}`. Apply the `DerivativeFunction` to `dataset1 <- cbind(A = 1:30, B = sample(1:100, 30))`.
...
* Obtain the median of each element of the list.
...
* Create a list that consist of the variable `transplant` from the **heart** data set and the variables `status` from the **retinopathy** data set. Give the name `list2` to this list. \
...
* Do you remember the practical `Writing your own function (Task 1 and 2)`? Now try to create again the same function (called `summary_df`) but avoid the use of a for loop. Apply the function to the **retinopathy** dat set.
...
<br>  &nbsp;&nbsp; paste0(round(mean(x), 1), " (", round(sd(x), 1), ")")
<br>}
<br><br>summary_categorical <- function(x) {
<br>  &nbsp;&nbsp; tab <- prop.table(table(x))
<br>  &nbsp;&nbsp; paste0(tab * 100, "% ", names(tab), collapse = ", ")
<br>}
</div>')
```
...
tapply(heart$year, list(heart$transplant, heart$surgery), function(x) {mean(x)/(length(x) - 2)})
