...
## Data Manupulation, Exploration and Visualization

Before starting with any statistical analysis it is important to transfoerm and explore your data set.

### Data Manupulation {.tabset .tabset-fade .tabset-pills}
...
Let's ontain some descriptive statistics.
...
Obtain the mean and sd for `age` only for transplanted patients using the data set `heart.`
...
* Obtain the median and interquartile range for `age` from the `retinopathy` data set. \ 
* Obtain the percentage per `type` from the `retinopathy` data set. \
...
Let's us now plot the data in order to undestand them even better.
...
* Create a scatterplot with the variables `age` and `year` using the data set `heart.` \
...
* Create a smooth plot or `age` and `risk` using the `retinopathy` data set. \
* Create a density plot or `age` per `type` group using the `retinopathy` data set.
